...
def fetch_builtin_jobs():
    ...
    try:
        ...
        print("BuiltIn scraping failed:", e)
    return jobs
...
def fetch_glassdoor_jobs():
    ...
    try:
        ...
        print("Glassdoor scraping failed:", e)
    return jobs
...
def main():
    ...
    df_new = pd.DataFrame(all_jobs, columns=["Company","Role","Location","Link","Date Posted"])
    if os.path.exists(CSV_FILE):
        df_existing = pd.read_csv(CSV_FILE)
        for col in ["Company","Role","Location","Link","Date Posted"]:
